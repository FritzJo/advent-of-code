...
function calculate_value($arr_winning_numbers, $arr_picked_numbers) {
    $correct_numbers = [];
...
foreach($lines as $line) {
    ...
    $duplication_stack[$card_id] += 1;
    $value = calculate_value($arr_winning_numbers, $arr_picked_numbers);
    for ($c = 0; $c < $duplication_stack[$card_id] ; $c++){
        for ($i = 1; $i <= $value; $i++){
            $duplication_stack[$card_id + $i] += 1;
        } 
    }
}
print_r($duplication_stack);
echo "Total: " . array_sum($duplication_stack);
?>